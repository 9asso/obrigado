...
class AppConfig {
  ...
  String get _runtimeOverridesCacheKey {
    ...
    return id.isEmpty
        ? 'appsgen_runtime_overrides'
        : 'appsgen_runtime_overrides_$id';
  }
  ...
  Uri? _buildRuntimeOverridesUri() {
    ...
    return Uri.parse(
        '$base/apps/${Uri.encodeComponent(id)}/runtime-overrides.json');
  }
    ...
    if (screens is Map) {
      ...
      for (final entry in screens.entries) {
        ...
        screensConfig[screenName] =
            (screensConfig[screenName] is Map<String, dynamic>)
                ? screensConfig[screenName]
                : <String, dynamic>{};
        (screensConfig[screenName] as Map<String, dynamic>)['enabled'] =
            enabled;
      }
    }
  ...
  // Game interstitial
  bool get gameInterstitialEnabled =>
      _config['game']?['interstitial']?['enabled'] ?? true;
  int get gameInterstitialInterval =>
      _config['game']?['interstitial']?['interval'] ?? 60;
  String get gameInterstitialCountdownPosition =>
      _config['game']?['interstitial']?['countdownPosition'] ?? 'topRight';
  ...
  double get subscriptionFooterFontSize =>
      (_config['subscription']['popup']['footer']?['style']?['fontSize'] ?? 12)
          .toDouble();
  Color get subscriptionFooterLinkColor => _parseColor(_config['subscription']
          ['popup']['footer']?['style']?['linkColor'] ??
      '#FFFFFF');
